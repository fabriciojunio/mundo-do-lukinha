...
import { generateHistoryQuizSet, checkHistoryAnswer, calculateHistoryScore, type HistoryQuiz } from './logic';
import { maquinaDoTempoConfig } from './config';
import { playSound } from '@/lib/sounds';
import { getRandomItem } from '@/lib/utils';
import { ENCOURAGEMENT_CORRECT, ENCOURAGEMENT_WRONG } from '@/lib/constants';

export function MaquinaDoTempoGame({ ageGroup, onGameEnd, onBack }: GameProps) {
  const quizzes = useRef<HistoryQuiz[]>(generateHistoryQuizSet(ageGroup));
  ...
  const handleAnswer = useCallback((selectedIndex: number) => {
    ...
    const isCorrect = checkHistoryAnswer(currentQuiz, selectedIndex);
    ...
    setTimeout(() => {
      if (qIndex + 1 >= totalQuizzes) {
        ...
        const r = calculateHistoryScore(newCorrect, totalQuizzes, timeSpent);
  ...
  const handlePlayAgain = useCallback(() => {
    quizzes.current = generateHistoryQuizSet(ageGroup);
    setQIndex(0); setCorrect(0); setFeedback(null); setPhase('playing'); setResult(null); startTime.current = Date.now();
  }, [ageGroup]);

  if (phase === 'finished' && result) {
    return <GameOverScreen result={result} gameName={maquinaDoTempoConfig.name} onPlayAgain={handlePlayAgain} onBack={onBack} />;
  }
  if (!currentQuiz) return null;

  return (
    <GameShell gameName={maquinaDoTempoConfig.name} gameIcon={maquinaDoTempoConfig.icon} score={correct * 15} onBack={onBack}>
      <div className="flex flex-col items-center justify-center h-full gap-5 p-6 max-w-lg mx-auto">
        ...
        <div className="bg-amber-50 rounded-2xl px-6 py-4 text-center w-full">
          <span className="text-4xl block mb-2">{currentQuiz.event.emoji}</span>
          <p className="text-xs font-body text-amber-600">{currentQuiz.event.era}</p>
        </div>
        ...
        <div className="flex flex-col gap-3 w-full">
          {currentQuiz.options.map((opt, i) => (
            <Button key={`${currentQuiz.id}-${i}`} variant="ghost" size="lg" fullWidth onClick={() => handleAnswer(i)} disabled={phase !== 'playing'}
              className="border border-gray-200 hover:border-amber-500 hover:bg-amber-50 text-left justify-start">
              <span className="font-display font-bold text-amber-700 mr-2">{String.fromCharCode(65 + i)}.</span>{opt}
            </Button>
          ))}
        </div>
        {feedback && (
